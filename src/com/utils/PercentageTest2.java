...
public class PercentageTest2 {
	...
	public static void main(String[] args) {
		...
		String str="5 % COPAY,4 % COPAY,14 % COPAY";
		
		String[] amts = str.split(",\\s*");
		List<String> values = Arrays.asList(amts);  
		
		
		List<String> dd= getCarrierCopayMenu (values);
		   for (String e : dd) {  
			   
			   
		        System.out.println("copay value :" + e + "\n");  
		      }  
	}
			...
			try{
				...
				if (copyToLocation > 0){
					...
					if (copyToLocation > 0){
						...
						copay = String.format( formatString, dbCopayValue);
					} else {
						formatString = "%1$s";
						copay = String.format( formatString,Integer.valueOf(copay.trim()));
					}
